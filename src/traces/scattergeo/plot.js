...
module.exports = function plot(geo, calcData) {
    ...
    gScatterGeoTraces.each(function(calcTrace) {
        ...
        if(subTypes.hasMarkers(trace)) {
            s.selectAll('path.point').data(calcTrace)
              .enter().append('path')
                .classed('point', true);
        }

        if(subTypes.hasText(trace)) {
            s.selectAll('g').data(calcTrace)
              .enter().append('g')
              .append('text');
        }
    });
...
function makeConvertToLonLatFn(trace, topojson) {
    ...
    return function(calcPt) {
        ...
        if(feature) calcPt.lonlat = feature.properties.ct;
        else calcPt.lonlat = [null, null];
